 ...
 * Stateless: makes the call, normalises the response. Persistence + grace
 * logic live in LicenseManager.
 *
 * Same protocol as vtinnovations/migrator; only the product code differs.
 */
final class LicenseVerifier
{
    // Canonical host: v-t.one 301-redirects to www.v-t.one, and Symfony
    // HttpClient downgrades a redirected POST to GET (dropping the JSON body),
    // so hit the final host directly.
    private const ENDPOINT   = 'https://www.v-t.one/api/v1/verify';
    private const PRODUCT    = 'vt-guardian';
    // Must match `vt_license_api_secret` on the v-t.one license server; sent
    // as the X-VT-Api-Key header.
